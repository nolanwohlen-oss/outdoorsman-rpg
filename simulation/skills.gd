extends RefCounted
## Phase 2Q progression ledger. XP is authoritative; levels are derived.
...
# stay at 1.00x in 2Q; later gates may extend them without changing these base IDs.
const ACTIONS := {
...
static func create() -> Dictionary:
	...
	for id in SUBSKILLS:
		angling[id] = {"xp": 0}
	return {"version": VERSION, "angling": angling, "rig_signatures": []}
...
static func validate(record: Variant) -> PackedStringArray:
	...
	if not record is Dictionary or record.size() != 3 or not record.has_all(["version", "angling", "rig_signatures"]):
	...
	if not signatures is Array or signatures.size() > 64:
		errors.append("Invalid rig XP guard record.")
	else:
		var seen: Dictionary = {}
		for signature in signatures:
			if not signature is String or signature.is_empty() or signature.length() > 256 or seen.has(signature):
...
static func rig_signature_is_new(record: Dictionary, signature: String) -> bool:
	return not signature.is_empty() and not record.rig_signatures.has(signature)

static func remember_rig_signature(record: Dictionary, signature: String) -> void:
	if signature.is_empty() or record.rig_signatures.has(signature):
		return
	record.rig_signatures.append(signature)
	if record.rig_signatures.size() > 64:
		record.rig_signatures.pop_front()
