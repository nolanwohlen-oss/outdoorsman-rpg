...
Lettered slices 2C–2M now cover deterministic environmental records, five-species population accounting, coarse player condition, the fishing state machine, physical item identity and storage, explicit fish processing, persistent tackle links, and the first timed fish-fight/landing causality loop. Each phase document records its own limits and phone gate. These are connected systems-lab fixtures, not completed versions of master phases 5–8.

Phase 2M must pass its Android checklist before another system depends on it. The next proposed slice is landing method and post-catch condition: explicit handling choices, release consequences and the first tackle condition/loss records without disguising placeholders as final physics. Camping, survival fidelity, individual wildlife behavior, the dedicated map and 3D presentation remain later gates. Passing automation does not substitute for physical Android testing or mean the complete initial survival loop is playable.
