...
if grep -E 'SCRIPT ERROR:|^ERROR:' "$project_root/build/import.log" "$project_root/build/tests.log" "$project_root/build/save-write.log" "$project_root/build/save-read.log"; then
  exit 1
fi
